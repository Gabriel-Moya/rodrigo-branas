...
test('Should not save post', async function () {
  const data = { title: generate(), content: generate() };
  const response1 = await request('http://localhost:3002/posts', 'POST', data);
  const response2 = await request('http://localhost:3002/posts', 'POST', data);
...
test('Should update a post', async function () {
  ...
  const response = await request(`http://localhost:3002/posts/${post.id}`, 'PUT', post);
...
test('Should delete a post', async function () {
  const post = await postsService.savePost({ title: generate(), content: generate() });
  const response = await request(`http://localhost:3002/posts/${post.id}`, 'DELETE');
  expect(response.status).toBe(204);
